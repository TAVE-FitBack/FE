import type { ConversionRate } from './data'
...
const BAR_GRADIENT: Record<ConversionRate['color'], string> = {
  coral: 'bg-gradient-to-r from-white/90 to-coral',
  lime: 'bg-gradient-to-r from-white/90 to-lime',
  blue: 'bg-gradient-to-l from-blue to-white',
}
  ...
  return (
    <div className="flex h-full flex-col gap-5 rounded-[28px] border border-gray-700 bg-gray-800 p-6">
      ...
      <div className="flex flex-col gap-4">
        {rates.map(({ key, label, percent, registered, consulted, color }) => (
          <div key={key} className="flex flex-col gap-1.5">
            <div className="relative h-9 w-full overflow-hidden rounded-full bg-black/30">
              <div
            ...
            <span className="text-caption-2 text-gray-500">
              등록 {registered} 건 / 상담 {consulted} 건
            </span>
          </div>
        ))}
      </div>
